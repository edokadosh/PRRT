...
const _ftype EPS = 1e-6;

struct Point {
    _ftype x, y;
    Point() {}
    Point(_ftype x, _ftype y): x(x), y(y) {}
    ...
    Point operator*(_ftype t) const {
        return Point(*this) *= t;
    }
    Point operator/(_ftype t) const {
        return Point(*this) /= t;
    }
    _ftype dot(const Point &t) const {
    	return (x*t.x + y*t.y); 
    }
    _ftype cross(const Point& t) const { 
        return x * t.y - y * t.x;
    }
    _ftype cross(const Point& a, const Point& b) const {
        return (a - *this).cross(b - *this); 
    }
    _ftype distance(const Point &t) const {
    	const double x_diff = x - t.x, y_diff = y - t.y ; 
    	return sqrt(x_diff * x_diff + y_diff * y_diff);
    }
    Point steer(const Point& t, _ftype DELTA) {
    ...
    Point normalize() 
    {
        return Point(*this) / Point(0,0).distance(*this);
    }
};

inline Point operator*(_ftype a, Point b) {
    return b * a;
}

inline _ftype distance(Point& a, Point &b) {
	const _ftype x_diff = a.x - b.x, y_diff = a.y - b.y ; 
	return sqrt(x_diff * x_diff + y_diff * y_diff);
}

inline _ftype dot(Point a, Point b) {
	return (a.x*b.x + a.y*b.y);
}

inline _ftype cross(Point a, Point b) {
    return (a.x*b.y - b.x*a.y); 
}

/*  Returns a point in the direction of (p2 - p1) vector such that 
    the new point is within a DELTA distance of point1  */
inline Point stepNear(Point& p1, Point& p2, _ftype DELTA) {
    if( (distance(p1, p2) - DELTA) <= EPS) 
        return p2 ; 
    else {
        _ftype theta = atan2(p2.y - p1.y, p2.x - p1.x);
        return Point(p1.x + DELTA * cos(theta), p1.y + DELTA * sin(theta));
    }
}

// Return minimum distance between line segment vw and point p
inline _ftype minimum_distance(Point v, Point w, Point p) {
  _ftype l2 = distance(v, w);  l2 *= l2 ; // i.e. |w-v|^2 -  avoid a sqrt
  ...
  const _ftype t = max(0.0, min(1.0, dot(p - v, w - v) / l2));
...
/*  Return true if the given line segment intersects the circle whose center
    is at location */
inline bool checkCollision(Point lineFrom, Point lineTo, Point location, _ftype radius)
{
...
inline bool PointInPolygon(Point point, Polygon polygon) {
...
// helper function
inline int sign(const _ftype x) { 
    return x >= 0 ? x ? 1 : 0 : -1; 
}
...
inline bool intersectOnLine(_ftype a, _ftype b, _ftype c, _ftype d) {
    if ((a - b) > EPS) swap(a, b);
    if ((c - d) > EPS) swap(c, d);
...
inline bool check_intersection(const Point a, const Point b, const Point c, const Point d) {
...
/*  Returns true if the given line segment represented by ba intersects with any
    side of the polygon */
inline bool lineSegmentIntersectsPolygon(Point a, Point b, Polygon& polygon) {
...
inline int yOfXOnLine(Point a, Point b, _ftype x) {
